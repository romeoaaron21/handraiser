module.exports = {
	helpStudent: (req, res) => {
		...
		db.query(`UPDATE requests SET status = 'inprogress' WHERE member_id=${memberid}`).then(() => {
			db
				.query(
	...
	movebacktoqueu: (req, res) => {
		const db = req.app.get('db');
		const { memberid, cohort_id } = req.params;
		db.query(`UPDATE requests SET status = 'waiting' WHERE member_id=${memberid}`).then(() => {
			db
				.query(
	...
	doneHelp: (req, res) => {
		const db = req.app.get('db');
		const { memberid, cohort_id } = req.params;
		db.query(`DELETE from requests WHERE member_id = ${memberid}`).then(() => {
			db
				.query(
