module.exports = {
	...
	doneHelp: (req, res) => {
		...
		db
			.query(`SELECT reasons.reason FROM requests LEFT JOIN reasons ON reasons.request_id = requests.id WHERE requests.member_id = ${memberid};`)
			.then(reason => {
				db.query(`INSERT INTO history (member_id, cohort_id, reason) VALUES (${memberid}, ${cohort_id}, '${reason[0].reason}')`)
				.then(() => {
					db.member.findOne({ student_id: memberid, cohort_id: cohort_id }).then(member => {
