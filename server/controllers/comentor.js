...
function fetchMentors(req, res) {
  const db = req.app.get("db");
  db.query(`SELECT * FROM users WHERE privilege = 'mentor' and id !=${req.params.mentor_id}`)
    .then(data => {
      res.status(201).json(data);
    })
    .catch(err => {
